...
class GnuPlot1 {
    ...
    public function push2($x, $y, $z, $index = 0) {
        $this->setY2active(true);
        if (!isset($this->values[$index])) {
            $this->values[$index] = [];
        }
    ...
    protected function getUsings() {
        ...
        if ($this->getY2active()) {
            for ($i = 0; $i < count($this->values); $i++) {
                ...
                $using = '"-" using 1:2 with linespoints pt ' . ($i + 1) . ' ps 2 axes x1y1';
            ...
            for ($i = 0; $i < count($this->values); $i++) {
                //            $using = '"-" using 1:2 with '.$this->mode;
                $using = '"-" using 1:2 with linespoints pt ' . ($i + 5) . ' ps 2 axes x1y2';
                if (isset($this->titles[$i])) {
                    $using .= ' title "' . $this->titles[$i] . '"';
                }
                $usings[] = $using;
            ...
            for ($i = 0; $i < count($this->values); $i++) {
                //            $using = '"-" using 1:2 with '.$this->mode;
                $using = '"-" using 1:2 with linespoints pt ' . ($i + 1) . ' ps 2';
                if (isset($this->titles[$i])) {
                    $using .= ' title "' . $this->titles[$i] . '"';
                }
    ...
    public function getX2label() {
        return $this->x2label;
    }

    public function getY2label() {
        return $this->y2label;
    }

    public function setX2label($x2label) {
    ...
    public function getY2active() {
        return $this->y2active;
    }

    public function setY2active($y2active) {
        $this->y2active = $y2active;
        return $this;
    }
